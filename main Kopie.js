...
function calcR(data, country) {
    let con = movingAvg(country_diff(data, country), 7)
    //country_data(data, country);
    let conSliced = con.slice(0, -4);
    return conSliced.map((e, i) => Math.round(con[i + 4] / e * 10) / 10);
    //return con.map((e, i) => e - (rec[i] + dea[i]));
}
...
function run(dates, confirmed, deaths, recovered) {
    ...
    setCBox({
        ...
        cb: context => {
            ...
            boxBarDraw(
                dates.slice(cdiff.length - cd.length),
                [
                    {
                        label: "Infections",
    ...
    draw_bar(dates.slice(cdiff.length - cd.length), [
        {
    ...
    draw(dates.slice(cdiff.length - cd.length), [
        {
            ...
            data: calcR(confirmed, "Germany").slice(cdiff.length - cd.length),
